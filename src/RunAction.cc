...
RunAction::RunAction()
{
    ...
    "H1",
    "Energy Deposit per event (MeV)",
    nBins, 0., 1.2*MeV
);

analysisManager->CreateH1("H2", "Photoelectron Energy", nBins, 0., 1*MeV);
analysisManager->CreateH1("H3", "Compton Electron Energy", nBins, 0., 1*MeV);
analysisManager->CreateH1("H4", "Electron Kinetic Energy", nBins, 0., 1*MeV);
...
void RunAction::BeginOfRunAction(const G4Run*)
{
    ...
    if (!argument) {analysisManager->OpenFile("output.root");
    return;
    }
